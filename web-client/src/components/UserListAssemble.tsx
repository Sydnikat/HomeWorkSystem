...
const UserListAssemble: React.FC<UserListAssembleProps> = (
  props: UserListAssembleProps
) => {
  const {
    listTitle,
    selectedListTitle,
  ...
  const [unSelectedUsers, setUnSelectedUsers] = useState<IUserResponse[]>(
    userList
  );
  const [preSelectedUserList, setPreSelectedUserList] = useState<
    IUserResponse[]
  >([]);
  ...
  useEffect(() => {
    setUnSelectedUsers(
      userList.filter(
        (s) =>
          selectedUsers.find((selected) => selected.userName === s.userName) ===
          undefined
      )
    );
